...
import com.theature.theaterdata.model.TheaterSection;
...
public class TheaterSeatingSearch {
    ...
    public List<PartyRequest> getTicketRequests(String ticketRequests) throws NumberFormatException{
        
        List<PartyRequest> requestsList = new ArrayList<PartyRequest>();
    ...
    private int findComplementRequest(List<PartyRequest> requests, int complementSeats, int currentRequestIndex){
        
        int requestNo = -1;

        for(int i=currentRequestIndex+1 ; i<requests.size() ; i++){
            
            PartyRequest request = requests.get(i);
    ...
    public void processTicketRequests(TheaterLayout layout, List<PartyRequest> requests) {
        ...
        for(int i=0 ; i<requests.size() ; i++){
            ...
            if(request.getNoOfTickets() > layout.getAvailableSeats()){
                //System.out.println("requested no of seats are not available in theater");
            ...
            for(int j=0 ; j<sections.size() ; j++){
                ...
                if(request.getNoOfTickets() == section.getAvailableSeats()){
                    ...
                    if(requestNo != -1){
                        ...
                        PartyRequest complementRequest = requests.get(requestNo);
        ...
        for(PartyRequest request : requests){
